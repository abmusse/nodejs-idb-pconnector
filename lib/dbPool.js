...
class DBPool {
  ...
  async runSql(sql = '') {
    ...
    const jobName = await connection.connection.getJobName().catch(async (error) => {
      reason = new Error('runSql: Failed to exec()');
      reason.stack += `\nCaused By:\n ${error.stack}`;
      console.error(error);
      // Remove the connection from the pool
      await this.retire(connection);
      console.log('recalling runsql');
  ...
  async prepareExecute(sql = '', params = [], options = {}) {
    ...
    const jobName = await connection.connection.getJobName().catch(async (error) => {
      reason = new Error('PE: Failed to exec()');
      reason.stack += `\nCaused By:\n ${error.stack}`;
      console.error(error);
      // Remove the connection from the pool
      await this.retire(connection);
      console.log('recalling prepare execute');
      await this.prepareExecute(sql, params, options);
    });
  ...
  // async __isJobActive__(connection)  {
